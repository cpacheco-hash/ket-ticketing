...
export function MusicPlayerSection({ artist }: MusicPlayerSectionProps) {
  ...
  return (
    <section className="music-player-section py-8">
      <h2 className="text-2xl font-bold mb-6">Escucha ahora</h2>
      ...
      {/* Player */}
      {activeTab === 'spotify' ? (
        <SpotifyPlayer spotifyId={artist.spotifyId} artistName={artist.name} />
      ) : (
        <YouTubePlayer artistName={artist.name} />
      )}
    </section>
  )
}
